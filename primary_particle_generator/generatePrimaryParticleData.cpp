...
/*
 * MeV kinetic energy of primary neutrino used if NEUTRINO_SPECTRUM_FLAG
...
/*
 * 0: gives mono-energetic neutrinos with
 ...
 * spectrum fit.
 * 1: gives neutrino spectrum in the range 1.8 ~ 10.0 MeV
 * according to spectrum fit of San Onofre reactor while ignoring
 * PRIMARY_NEUTRINO_KINETIC_ENERGY.
 * 2: gives flat neutrino spectrum from 1.8 to X MeV in increments of keV for
 * ITERATIONS iterations.
 */
#define NEUTRINO_SPECTRUM_FLAG 2
...
int main(int argc, char** argv)
{
	...
	std::ofstream fout ("primaryParticleData.dat");
...
//	cout << header << endl;
	...
	for(int i = 0; i < ITERATIONS;){
		//cout << "i = " << i << endl;
#if NEUTRINO_SPECTRUM_FLAG==0
		const double Enu = PRIMARY_NEUTRINO_KINETIC_ENERGY; // Constant neutrino kinetic energy (MeV).
#elif NEUTRINO_SPECTRUM_FLAG==1
		double Enu = randGen.Uniform(1.8, 10.); // Neutrino kinetic energy (MeV).
		double randomSampleValue_1 = randGen.Uniform(0., 1.);
		if(randomSampleValue_1 > GetNeutrinoSpectrum(Enu)) continue;
#elif NEUTRINO_SPECTRUM_FLAG==2
		double Enu = 1.800 + 0.001*i;
#else
		std::cerr << "NEUTRINO_SPECTRUM_FLAG preprocessor directive "
			"value is invalid" << std::endl;
		abort();
#endif
		cosThetaPositronWRTNeutrinoMomentumDirection =	randGen.Uniform(-1., 1.); // cosThetaPositronWRTNeutrinoMomentumDirection = Cos(theta_e).
		double randomSampleValue_2 = randGen.Uniform(0., 1.); // Random sample between 0. ~ 1.
		...
		fout << std::setw(10) << i << " "
		   	<< std::setw(6) << Enu << " "
		   	<< std::setw(15) << cosThetaPositronWRTNeutrinoMomentumDirection << " "
			<< std::setw(15) << KEPositron << " "
			<< std::setw(15) << cosThetaNeutronWRTNeutrinoMomentumDirection << " "
		   	<< std::setw(15) << KENeutron << endl;
		i++;
